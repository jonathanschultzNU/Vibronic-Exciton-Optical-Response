% DIMHAMGEN  Generate the vibronic dimer Hamiltonian and transition dipole operators.
%
%   HSTRUCT = DIMHAMGEN(PAR) constructs the full Frenkel-exciton vibronic Hamiltonian
%   for a molecular dimer and returns all relevant sub-blocks and operators in a struct.
%
%   The Hamiltonian includes:
%     - Site energies (e1, e2) for molecules 1 and 2
%     - Dipolar coupling J between molecules
%     - Harmonic vibrational mode w0 on each molecule
%     - Linear vibronic coupling lambda (Holstein coupling)
%
%   The basis is organized as: [ground manifold | single-excitation manifold | double-excitation manifold]
%   Each basis state is a 4-vector: [elec1, vib1, elec2, vib2]
%
%   Input:
%     PAR     - parameter struct with fields: e1, e2, w0, J, lambda, v_max, c
%
%   Output:
%     HSTRUCT - struct containing:
%       .Hgg, .Hggwvn   - ground-state Hamiltonian (frequency and wavenumber units)
%       .Hse, .Hsewvn   - single-excitation Hamiltonian
%       .Hde, .Hdewvn   - double-excitation Hamiltonian
%       .Hwvn           - full Hamiltonian in wavenumber units
%       .Hdiag          - eigenvalue matrix
%       .eigvec         - eigenvector matrix
%       .fockgg/se/de   - Fock space identity blocks
%       .fock, .fock_e  - full Fock space (site and exciton bases)
%       .MUg_se         - transition dipole: ground -> single-excitation
%       .MUse_g         - transition dipole: single-excitation -> ground
%       .MUse_de        - transition dipole: single -> double-excitation
%       .MUde_se        - transition dipole: double -> single-excitation
%       .MU             - full transition dipole operator
%       .mueig          - transition dipole in eigenbasis
%
%   See also: VIBCRE, ELCRE, VIBRON_ZERO_EXC, VIBRON_SINGLE_EXC, VIBRON_DOUBLE_EXC
...
mu1E = 1;   
mu2E = 1;   %set arbitrary scaling factor for influence of electric field 
